/**
 * Self-test for the agent brain — no external AI APIs or telephony required, but it does
 * use the database. Injects a stubbed LLM into the Conversation engine and drives a full
 * "book an appointment" flow, asserting the tool loop + calendar work.
 *
 *   DATABASE_URL=postgres://... npm run selftest
 */
...
async function main() {
  await runMigrations();
  const agent = await createAgent({ name: 'Selftest', business_name: 'Test Co' } as any);
  const callId = await createCall({ agent_id: agent.id, direction: 'inbound', from_number: 'selftest' });
  const ctx: ToolContext = { agent, callId, contact: {}, callerNumber: '+15125559999' };
  const conv = new Conversation(new StubLlm(), agent, ctx);

  const before = (await listAppointments({ agentId: agent.id })).length;
  const replies: string[] = [];
  for await (const ev of conv.respondTo('I want to book a move for my apartment')) {
    if (ev.type === 'say') replies.push(ev.text);
  }
  const after = (await listAppointments({ agentId: agent.id })).length;
