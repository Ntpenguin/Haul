...
| `supabase/migrations/010–053` | Incremental. Notables: quote_requests/leads (017–038), payouts/Connect (032), admin RPCs + `is_admin()` (034), RLS hardening (039/040), rate limiter (041), **difficulty + duration + skill gate (042)**, **businesses + fleet (043/044)**, **surcharges (045)**, admin-insert-gigs (046), **referral program (047, payout_handle 051)**, **B2B intake jobs (048; special_items 050; booked-slot union 052; payment 053)**, **Single-item difficulty (049)**. Forward-only; run manually in the SQL editor; use named `$func$` quoting. |
...
- **Tabs**: **Referrals** (codes + QR + Venmo/Zelle payout + owed-payout table), **Business Jobs** (B2B intake review: counter / book→bills / complete; click a row for the detail modal), **Pricing** (read-only rate reference). **Add gig** button on the Gigs tab manually inserts a gig (migration 046 INSERT policy).
...
- **CSP**: `img-src` must allow `https://api.qrserver.com` (referral QR images) — else they render as broken icons.
